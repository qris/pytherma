...
class DecodingTest(unittest.TestCase):
    def test_cases(self):
        ...
        self.assertEqual(37.1, values[144][1], "Wrong decode for 144:Leaving water temp. before BUH (R1T)(C)")

        # 4008,4019 22:46:21.4443689 [(7, 112, 111)] (36.8 => 36.7)
        values = decoding.decode(bytes([3, 64, 97, 91]),
                                 bytes([64, 97, 18, 128, 0, 116, 1, 111, 1, 37, 1, 67, 1, 18, 2, 250, 0, 0, 0, 111]))
        self.assertEqual(37.2, values[144][1], "Wrong decode for 144:Leaving water temp. before BUH (R1T)(C)")
        self.assertEqual(36.7, values[145][1], "Wrong decode for 145:Leaving water temp. after BUH (R2T)(C)")
        self.assertEqual(29.3, values[146][1], "Wrong decode for 146:Refrig. Temp. liquid side (R3T)(C)")
        self.assertEqual(32.3, values[147][1], "Wrong decode for 147:Inlet water temp.(R4T)(C)")
        self.assertEqual(53.0, values[148][1], "Wrong decode for 148:DHW tank temp. (R5T)(C)")
        ...
        self.assertEqual(True, values[156][1], "Wrong decode for 156:Powerful DHW Operation. ON/OFF")
        ...
        self.assertEqual(94, values[181][1], "Wrong decode for 181:Water pump signal (0:max-100:stop)")

    def test_overlap(self):
        ''' For each command, make sure that decoders do not overlap. Only bit decoders can test
            the same byte(s) of the response, and those must not be testing the same bit.
        '''
